...
// Best-effort in-memory rate limit (single replica).
...
export async function POST(req: NextRequest) {
  ...
  if (!crmConfigured()) {
    console.warn("[contact] Relaticle not configured; inquiry not sent to CRM:", {
      topic,
      name,
      email,
  ...
  try {
    ...
    await alertOps(
      `verana.io contact: CRM write failed for ${name} <${email}> (${topic}). ${String(
        err
      ).slice(0, 300)}`
    );
